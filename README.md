...
2. **Scoring.** Each new comment is scored by a local toxicity classifier
   ...
   access on a fresh runner) or fails on a specific comment, it falls back
   to an offline keyword check using the maintained
...
6. The first run will download the toxicity model (~270MB) and backfill
